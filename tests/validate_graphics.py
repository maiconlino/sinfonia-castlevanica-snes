...
from refined_art import indexed, CASTLE_PIXELS, HERO_PIXELS, shared_tiles
...
def main():
 for r in range(8):
  ...
  assert reconstructed.tobytes()==(indexed(CASTLE_PIXELS,(256,224)) if r==0 else background(r)).tobytes(),f'region{r} mismatch'
  assert all(v<32768 for v in struct.unpack('<16H',(OUT/f'bg_region{r}.pal').read_bytes()))
 for i,tile in enumerate(shared_tiles(common_tiles())):assert decode_tile(planar(tile)).tobytes()==tile.tobytes(),f'common{i}'
 ...
 frames=(OUT/'hero-frames.chr').read_bytes();assert len(frames)==16*768
 poses=indexed(HERO_PIXELS,(256,96))
 for f in range(16):
  decoded=Image.new('P',(32,48))
  for y in range(6):
   for x in range(4):
    at=f*768+(y*4+x)*32
    decoded.paste(decode_tile(frames[at:at+32]),(x*8,y*8))
  assert decoded.tobytes()==poses.crop(((f%8)*32,(f//8)*48,(f%8)*32+32,(f//8)*48+48)).tobytes()
 print('PASS: 8 background maps, 224 common tiles,16 dynamic 32x48 hero poses,10bosses,16-bit CGRAM palettes round-trip exactly.')
