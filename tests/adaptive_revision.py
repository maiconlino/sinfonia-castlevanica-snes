...
OUT=ROOT/'build/qa-revision';OUT.mkdir(parents=True,exist_ok=True)
...
def run(n=1,buttons=()):
 ...
 read()
 if e.frame>120000:raise AssertionError('Bounded campaign test exceeded frame budget')
...
def fight():
 for f in range(2600):
  ...
  if f%43==0 and val('mp')>=15:buttons+=['A']
...
def travel(goal):
 for old,j,target in path(goal):
  assert val('room')==old
  changeform(0);move_to(T['door_x'][j]-8)
  if T['room_exit_secret'][old*4+j]:run(35,['Y'])
  form=next((f for i,f in [(32,1),(34,2),(35,3)]if i in reqs(old,j)),0)
  changeform(form);tap('UP');run(6)
  assert val('room')==target,('Door failed',old,j,target,val('room'),val('py'),val('messageitem'),val('form'))
  if val('bosshp'):fight()
  if val('mode')==3:return
  # Passing a populated room can be dangerous; clear it before the next leg.
  fight()
