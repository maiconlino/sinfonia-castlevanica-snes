...
## Revisão 2.0: movimento, animação e portas

A revisão 2.0 substitui o personagem humano 16 × 32 por um metasprite 32 × 48, com oito poses de caminhada e poses próprias para repouso, ataque, salto, queda e esquiva. Cabelo, casaco e capa têm camadas de sombreamento. Cada porta é um desenho único 32 × 48 montado com 24 tiles, e não quatro miniportas repetidas. As oito regiões receberam cenários e paletas revistos. O HUD agora usa duas linhas e a faixa preta inferior permanente foi removida.

Movimento horizontal em ponto fixo 8.8 com aceleração e frenagem, salto de altura variável, tolerância de cinco quadros ao sair de uma plataforma e buffer de seis quadros para o salto. Não há aumento artificial da resolução do SNES.

Baixe a ROM `.smc` em `releases/v2.0/`. A versão anterior está preservada em `releases/v1-original/` ou no histórico de preservação original. Inicie a nova ROM normalmente: estados instantâneos do emulador feitos com outra ROM não fazem parte dos testes desta revisão. O teste de SRAM da versão 2 verificou salvar no santuário e continuar após uma inicialização nova.

**Validação:** consulte `docs/REVISION-2.md`. A sequência temporizada do percurso completo da versão antiga não concluiu a versão 2 após a alteração da física. Isso não estabelece um bloqueio do mapa, mas impede afirmar que a campanha inteira da nova revisão foi validada. Não foi realizado teste em console físico.
...
Requisitos: Python 3 e `ca65`/`ld65` do cc65 disponíveis no PATH. Os binários dos gráficos, da música e dos dados já acompanham o projeto. Pillow e NumPy são necessários apenas para regenerar os assets e executar os testes gráficos.
...
python3 tools/remaster_graphics.py
python3 tools/build_audio.py
python3 tests/validate_graphics.py
python3 tools/build.py
```

O processo de publicação regenera os assets, compila a ROM e compara seu SHA-256 com o build local da revisão antes de publicar.
...
| `tools/remaster_graphics.py` | Pixel art e layout da revisão 2; usa os helpers de `generate_graphics.py` |
