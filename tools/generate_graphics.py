...
def main():
 common=common_tiles();shared=b''.join(planar(t) for t in common)
 ...
 for r,(name,label,_) in enumerate(regions):
  ...
  manifest['regions'].append({'id':r,'name':name,'tiles':128+len(tiles),'chrBytes':32768,'mapBytes':2048,'paletteBytes':32})
